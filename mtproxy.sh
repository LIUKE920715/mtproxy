...
Set_passwd(){
    ...
    echo -e "  ${Green}2.${Nc} 安全模式 (dd+32位十六进制字符)${Red_globa}[默认]${Nc}"
    echo -e "  ${Green}3.${Nc} TLS 模式 (ee+32位十六进制字符+TLS域名)"
    
    read -e -p "(请输入数字 1-3，默认：2):" mtp_mode
    [[ -z "${mtp_mode}" ]] && mtp_mode="2"
    ...
    case "${mtp_mode}" in
        1)
            ...
            sed -i 's/^#\?.*classic.*/    "classic": True,/g' $mtp_conf
        ...
        2)
            # 安全模式
            sed -i 's/^#\?.*user".*/    "user": "'"$mtp_passwd"'",/g' $mtp_conf
            sed -i 's/^#\?.*classic.*/    "classic": False,/g' $mtp_conf
